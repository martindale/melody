...
melody.use(require('maki-client-traverse'));
melody.use(identity);
melody.use(auth);
//melody.use(fabric);
...
Post.pre('create', function lengthValidator(next, done) {
  var post = this;
  if (!post.content || !post.content.length) {
    return done('New posts must have content.');
  } else {
    return next();
  }
});
...
Post.pre('create', function(next, done) {
  ...
  Person.get(query, function(err, person) {
    ...
    if (!person) {
      console.error('No person found for Post.pre("create", ...), query was:', query);
      return done('No such author.');
    }
